...
export const signUp = function (req, res) {
  ...
  if (!id || !name || !password || !email || !interest) {
    res.json({ code: 500, v: 'v1', status: 'ERR', detail: 'INVALID FORMAT'})
  } else {
    async.waterfall([
      (callback) => {
        ...
        connection.query(sql, [], (err, result) => {
          if (err) {
            ...
            if (result[0].count > 0) {
              callback({err: 'ERR_SIGNUP', message: 'USERID ALREADY EXISTS'})
            } else {
              callback(null, '')
            }
          }
    ...
    (err, result) => {
      if (err) {
        ...
        res.json({ code: 200, v: 'v1', status: 'SUCCESS', detail: 'Sign up successful!' })
      }
    })
  }
}
  ...
  if (!id || !password) {
      ...
      (callback) => {
        ...
        connection.query(sql, [], (err, result) => {
          if (err) {
            ...
            if (result[0].count == 0) {
              callback({err: 'ERR_SIGNIN', message: 'INVALID PASSWORD OR ID'})
            } else {
              callback(null, '')
            }
          }
    ...
    (err, result) => {
      if (err) {
        ...
        res.json({ code: 200, v: 'v1', status: 'SUCCESS', detail: 'Sign in successful!' })
      }
    })
  }
}
